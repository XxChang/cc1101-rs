...
fn main() -> ! {
    ...
    loop {
        cc1101.clear_rx_fifo().unwrap() ;
        cc1101.start_receive().unwrap() ;
        while !cc1101.check_receive(false).unwrap() {
            cc1101.delay_us(1000);
        }
        let mut buff = [0u8;256];
        let len = cc1101.get_received(&mut buff).unwrap() ;
        if len.0 != 0 {
            led.toggle() ;
            // for d in &buff[..len.0 as usize] {
            //     rprintln!("{:02x}", d);
            // }
            rprintln!("{:?}", &buff[..len.0]);
        }
